...
def main():
    ...
    if 'selected_batter' not in st.session_state:
        st.session_state.selected_batter = sorted(df["bat"].dropna().unique())[0]
    if 'year_range' not in st.session_state:
        st.session_state.year_range = (int(df["year"].min()), int(df["year"].max())) if "year" in df.columns else None
    if 'bowler_type' not in st.session_state:
        st.session_state.bowler_type = "All"

    # Sidebar inputs
    selected_batter = st.sidebar.selectbox("Select Batter", sorted(df["bat"].dropna().unique()), index=sorted(df["bat"].dropna().unique()).index(st.session_state.selected_batter))
    if "year" in df.columns:
        minY = int(df["year"].min())
        maxY = int(df["year"].max())
        year_range = st.sidebar.slider("Year Range", minY, maxY, st.session_state.year_range)
    else:
        year_range = None
    bowler_type = st.sidebar.radio("Bowler Type", ["All", "Spin", "Pace"], index=["All", "Spin", "Pace"].index(st.session_state.bowler_type))
    if st.sidebar.button("Generate"):
        st.session_state.selected_batter = selected_batter
        st.session_state.year_range = year_range
        st.session_state.bowler_type = bowler_type

    # Apply filters only when Generate is clicked
    if 'generate_clicked' not in st.session_state:
        st.session_state.generate_clicked = False
    if st.session_state.generate_clicked or st.sidebar.button("Generate"):
        st.session_state.generate_clicked = True
        sub = df[df["bat"] == st.session_state.selected_batter].copy()
        if st.session_state.year_range and "year" in sub.columns:
            sub = sub[(sub["year"] >= st.session_state.year_range[0]) & (sub["year"] <= st.session_state.year_range[1])]
        if st.session_state.bowler_type != "All":
            sub = sub[sub["bowler_type"] == st.session_state.bowler_type]
        ...
        with analysis_tab:
            ...
            <div style='background-color:#1e1e1e; color:white; padding:20px; border-radius:5px; margin-bottom:20px;'>
              <h2>{st.session_state.selected_batter} | {hand}</h2>
              <p>
                Runs: {total_runs} | 
                Balls: {balls_faced} | 
                SR: {sr_val:.2f} | 
            ...
            with tab1:
                ...
                if not bdf.empty:
                    ...
                    st.markdown(f"""
                    **Insights**:
                    - **Boundary Distribution**: {st.session_state.selected_batter} hit {fours} fours and {sixes} sixes, totaling {total_boundaries} boundaries.
                    ...
                    - **Side Preference**: {st.session_state.selected_batter} favors the {side_preference}, with {off_side_pct:.1f}% of boundaries on the off-side and {leg_side_pct:.1f}% on the leg-side.
                    - **Shot Difficulty**: The intelligent wagon wheel highlights Zone {hardest_zone} as the area where {st.session_state.selected_batter} excels in difficult shots (average weighted score: {hardest_zone_value:.1f}).
                    - **Takeaway**: {st.session_state.selected_batter} could focus on diversifying shot selection to reduce predictability, while bowlers might target less dominant zones to restrict boundary scoring.
                    """)
                else:
                ...
                if not zone_df.empty:
                    ...
                    st.markdown(f"""
                    **Insights**:
                    - **Top Scoring Zone**: Zone {top_zone_runs_id} is {st.session_state.selected_batter}'s most productive, contributing {top_zone_runs_value} runs ({top_zone_runs_pct:.1f}% of total runs).
                    ...
                    - **Takeaway**: {st.session_state.selected_batter} might target Zone {weak_zone_id} to improve scoring, while bowlers could exploit Zone {high_dismissal_zone_id} to increase dismissal chances.
                    """)
                else:
                    st.markdown("**Insights**: No valid zone data available for this selection.")

            with tab2:
                st.subheader(f"Scoring Patterns Over Time for {st.session_state.selected_batter}")
                if 'year' in sub.columns:
                    ...
                    plt.title(f'Strike Rate and Runs Over Time for {st.session_state.selected_batter}')
                    plt.tight_layout()
                    st.pyplot(fig6)
                    ...
                    plt.title(f'Dismissals Per Year for {st.session_state.selected_batter}')
                    plt.xlabel('Year')
                    ...
                    if not yearly_stats.empty:
                        ...
                        st.markdown(f"""
                        **Insights**:
                        - **Peak Strike Rate**: {st.session_state.selected_batter} achieved their highest strike rate of {peak_sr_value:.1f} in {peak_sr_year}.
                        - **Peak Scoring Year**: The most runs ({peak_runs_value}) were scored in {peak_runs_year}.
                        - **Highest Dismissals**: {st.session_state.selected_batter} was dismissed {max_dismissals_count} times in {max_dismissals_year}, indicating a challenging year.
                        - **Takeaway**: {st.session_state.selected_batter} should aim to replicate their {peak_sr_year} form, while bowlers can exploit recent trends (e.g., higher dismissals in {max_dismissals_year}) to target weaknesses.
                        """)
                else:
                    st.markdown("**Error**: 'year' column not found.")

            with tab3:
                st.subheader(f"Percentage of Outs by Bowl Length for {st.session_state.selected_batter}")
                length_summary = (
                ...
                plt.title(f'Percentage of Outs by Bowl Length for {st.session_state.selected_batter}', fontsize=14, fontweight='bold')
                ...
                if total_outs > 0:
                    ...
                    st.markdown(f"""
                    **Insights**:
                    - **Most Dangerous Length**: {max_length} deliveries dismiss {st.session_state.selected_batter} most often ({max_pct:.1f}% of outs, {int(max_balls)} balls faced), making it the best length to bowl.
                    ...
                    - **Takeaway**: Bowlers should prioritize {max_length} deliveries to maximize dismissal chances, while {st.session_state.selected_batter} might focus on improving technique against this length.
                    """)
                else:
                    st.markdown(f"""
                    **Insights**: No dismissals recorded for {st.session_state.selected_batter} under the current filters.  
                    Try adjusting the filters to see dismissal patterns.
                    """)

                st.subheader(f"Percentage of Outs by Bowl Line for {st.session_state.selected_batter}")
                if 'line' in sub.columns:
                    ...
                    plt.title(f'Percentage of Outs by Bowl Line for {st.session_state.selected_batter}')
                    ...
                    if total_outs > 0:
                        ...
                        st.markdown(f"""
                        **Insights**:
                        - **Most Dangerous Line**: {max_line} deliveries are most likely to dismiss {st.session_state.selected_batter} ({max_pct:.1f}% of outs, {max_balls} balls faced), making it the best line to target.
                        - **Safest Line**: {min_line} deliveries are least effective, with only {min_pct:.1f}% of dismissals ({min_balls} balls faced).
                        - **Takeaway**: Bowlers should focus on the {max_line} line to increase dismissal chances, while {st.session_state.selected_batter} might work on defending or attacking this line more effectively.
                        """)
                    else:
                        st.markdown(f"""
                        **Insights**: No dismissals recorded for {st.session_state.selected_batter} under the current filters.  
                ...
                st.subheader(f"Percentage of Outs by Bowl Line and Length for {st.session_state.selected_batter}")
                if 'line' in sub.columns and 'length' in sub.columns:
                    ...
                    if not sub_filtered.empty:
                        ...
                        if total_outs > 0:
                            ...
                            sns.heatmap(pivot_outs_percent, annot=True, cmap='YlOrRd', fmt='.1f', cbar_kws={'label': 'Outs (%)'},
                                        ax=ax5, linewidths=1, linecolor='gray', alpha=1.0)
                            plt.text(2.5, 3, f'No dismissals for {st.session_state.selected_batter}', ha='center', va='center', fontsize=12, color='black')
                        plt.title(f'Percentage of Outs by Bowl Line and Length for {st.session_state.selected_batter}', fontsize=14, fontweight='bold', pad=20)
                        ...
                        if total_outs > 0:
                            ...
                            st.markdown(f"""
                            **Insights**:
                            - **Most Dangerous Combination**: The {max_line} {max_length} combination is most effective, accounting for {max_pct:.1f}% of {st.session_state.selected_batter}'s dismissals ({max_balls} balls faced).
                            {min_text}
                            - **Takeaway**: Bowlers should target the {max_line} {max_length} combination to maximize dismissal chances, while {st.session_state.selected_batter} should be cautious against this combination.
                            """)
                        else:
                            st.markdown(f"""
                            **Insights**: No dismissals recorded for {st.session_state.selected_batter} under the current filters.  
            ...
            with tab4:
                ...
                if not sty_table.empty:
                    ...
                    if not non_zero_runs.empty:
                        ...
                        min_sr_text = f"- **Least Comfortable Style**: {st.session_state.selected_batter} struggles against {min_sr_style}, with a strike rate of {min_sr_value:.1f} over {min_sr_balls} balls."
                    else:
                    ...
                    st.markdown(f"""
                    **Insights**:
                    - **Most Comfortable Style**: {st.session_state.selected_batter} scores freely against {max_sr_style}, with a strike rate of {max_sr_value:.1f} over {max_sr_balls} balls.
                    {min_sr_text}
                    - **Most Dangerous Style**: {max_dismiss_style} has dismissed {st.session_state.selected_batter} {max_dismiss_count} times over {max_dismiss_balls} balls, making it the most effective.
                    - **Most Aggressive Against**: {st.session_state.selected_batter} hits boundaries most frequently against {max_boundary_style}, with a boundary percentage of {max_boundary_pct:.1f}% over {max_boundary_balls} balls.
                    - **Most Restrictive Style**: {max_dot_style} restricts {st.session_state.selected_batter} the most, with a dot ball percentage of {max_dot_pct:.1f}% over {max_dot_balls} balls.
                    - **Highest Impact Style**: {max_impact_style} has the highest impact against {st.session_state.selected_batter}, with an impact of {max_impact_value:.1f} per 100 balls over {max_impact_balls} balls.
                    - **Takeaway**: {st.session_state.selected_batter} should work on improving scoring against {max_dot_style} to reduce dot balls and be cautious against {max_dismiss_style} to avoid dismissals. Bowlers should use {max_dismiss_style} to target {st.session_state.selected_batter}'s weaknesses.
                    """)
                else:
                    st.write("No bowling style data found.")

            with tab5:
                st.subheader(f"Match-Up Analysis by Phase and Bowling Style: {st.session_state.selected_batter}")
            ...
            with tab6:
                ...
                if missing_cols:
                    ...
                    if df_model.empty:
                        ...
                        st.write(f"**Will {st.session_state.selected_batter} get out?** {'🟥 Yes' if prediction == 1 else '🟩 No'}")
